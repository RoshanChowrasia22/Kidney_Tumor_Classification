...
class PrepareBaseModel:
    ...
    def _prepare_full_model(model,classes,freeze_all,freeze_till,learning_rate):
        # This part is where we free the layers basically transfer learning
        if freeze_all:
            for layer in model.layers:
                layer.trainable=False
        elif (freeze_till!=None) and freeze_till>0:
            for layer in model.layers[:-freeze_till]:
                layer.trainable=False
        
        flatten_in=tf.keras.layers.Flatten()(model.output)
        hidden_layer=tf.keras.layers.Dense(units=1080,activation=tf.nn.gelu)(flatten_in)
        prediction=tf.keras.layers.Dense(units=classes,activation="softmax")(hidden_layer)
